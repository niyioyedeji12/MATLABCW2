...
for t = 1:readingsNum
    voltage(t) = readVoltage(a, sensorPin); % read voltage from sensor
    temp(t) = (voltage(t) - V0C) / TC; % convert voltage to temperature
    pause(timeInterval); % 1 second between readings
end

disp('Data acquisition finished.')

minTemp = min(temp); % finds minimum temperature
maxTemp = max(temp); % finds max temperature
avgTemp = mean(temp); % finds average temperature

figure;
plot(time, temp, 'b-', 'LineWidth', 1.5); % plot temperature against time
xlabel('Time (s)');
ylabel('Temperature (°C)');
title('Temperature vs Time');
grid on;

dateStr = input('What date is it today? (use the format dd/mm/yyyy): ', 's'); % input for date
locationStr = input('Where are you located?: ', 's'); % input for location
fprintf('Data Logging Initated - %s\n', dateStr);
fprintf('Location - %s\n\n', locationStr);

for t = 0:10 % display temp at 1 minute intervals
...
% fileContents = fread(checkID, '*char')'; 
% fclose(checkID);
...
% Task 1 was about reading the thermistor, converting voltages to temperatures, and plotting data. Getting the log file format right with fprintf was hard and took many tries and understanding how to use the file function took some work and research. 
...
% I also struggled a lot with saving using github as very often despite
% committing changes, it would not save properly. I also got better at
% debugging MATLAB code, however, wiring circuits was where I struggled. In
% the future, I’d practice building circuits more to get faster write some
% test codes to better help verifying whether my code and circuit work. I
% could also add some error checking where I used inputs.
